					...
					$("#code").blur(function(e) {
						...
						$.ajax({
							...
							error : function(data) {
								code_yz=0;
								$("#login_alert").empty();
								$("#login_alert").append("验证码错误，请重新输入"+code);
							}
					    });
					});
					/*登陆*/
								...
								$.ajax({
									...
									success : function(data) {
										if (data.S == "success") {
											...
											window.location.href = APP + "Index/index_main&uid=" + data.uid + "&type=" + data.utype + "&from=0&ustate="+data.ustate;
										}
										if (data.S == "failed") {
											$("#login_alert").attr("class", "alert alert-danger");
											$("#login_alert").append("" + data.MT + "");
										}
									},
									error : function() {
										$("#login_alert").attr("class", "alert alert-danger");
